...
from datapipeline.LOLdataset import LOLDataset
...
def main(basic_opt):
    ...
    if opt.phase=="train":
        ...
        if opt.comment != "None":
            opt.exp_name = opt.exp_name+"_"+opt.model+"_"+opt.adb_type+"_%d"%opt.num_orders+"_"+opt.comment+"_"+current_time
        else:
    ...
    # opt.test_path = opt.testset

    model = Model(opt)
    if opt.phase=="train":
        print("train")
        train_data = LOLDataset(data_dir=opt.data_path, mode='train')
        train_dataloader = torch.utils.data.DataLoader(train_data,
                                                       batch_size=opt.train_bsz,
                                                       shuffle=True,
                                                       num_workers=opt.num_workers,
                                                       pin_memory=True)
        eval_data = LOLDataset(data_dir=opt.data_path, mode='eval')
